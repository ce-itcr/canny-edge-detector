/*
  ...
  SUMA, MULTI, DIV, LOAD, STORE, CMP, SHIFT, XOR
  ADD , MUL  , DIV,  MOV,   STR, CMP, 
  ADD ,
*/
...
#define TYPE_MULTIPLY 2 //mul,
#define TYPE_DIV 3 //new div
#define TYPE_STR 4 //new push, pop
#define TYPE_CMP 5 //new cmp, je, jmp, jg, jl, jge, jne, jz
#define TYPE_XOR 6 //new xor
#define LOAD_TIMEOUT 1
#define SUM_TIMEOUT 1 
#define MULTIPLY_TIMEOUT 2
#define DIV_TIMEOUT 10
#define STR_TIMEOUT 10
#define CMP_TIMEOUT 1
#define XOR_TIMEOUT 1
#define LOAD_PW 5 //1 a 10W
#define SUM_PW 1 //0.5 5 W
#define MULTIPLY_PW 2 // 2W
#define DIV_PW 2
#define STR_PW 5
#define CMP_PW 1
#define XOR_PW 1
...
double cpu_time_total = 0.0;  // Variable global para acumular el tiempo de ejecución
...
/*************************** Functions to parse intructions ********************************/

char* get_source_register(const char* instruction) {
        char* source_reg = NULL;
        const char* pattern = "%[^,], %[^\n ]";

        if (strstr(instruction, ",") != NULL) {
            source_reg = malloc(10 * sizeof(char)); // Corrección: se añade * antes de sizeof(char)
            sscanf(instruction, pattern, source_reg);
            return source_reg;
        }
...
char* get_destination_register(const char* instruction) {
        char* dest_reg = NULL;
        const char* pattern = "%[^\n ,]%[ ,]%[^ ,\n]";

        if (strstr(instruction, ",") != NULL) {
            dest_reg = malloc(32 * sizeof(char)); // Corrección: se añade * antes de sizeof(char)
            sscanf(instruction, pattern, dest_reg);
        }

        return dest_reg;
}

int obtener_registro_destino(char *disassembled_text) {
    char *token = strtok(disassembled_text, ",");
    if (token == NULL) {
        // No se encontró una coma en la cadena
        return -1; // O cualquier otro valor de error que desees utilizar
    }

    // Buscar la cadena "add" en el token
    char *add_token = strstr(token, "add");
    if (add_token == NULL) {
        // No se encontró la cadena "add"
        return -1; // O cualquier otro valor de error que desees utilizar
    }

    // Avanzar hasta el primer espacio después de la cadena "add"
    add_token += strlen("add");
    while (*add_token == ' ') {
        add_token++;
    }

    // Buscar el registro de destino en el token
    char *register_token = strstr(add_token, "r");
    if (register_token == NULL) {
        // No se encontró la cadena "r"
        return -1; // O cualquier otro valor de error que desees utilizar
    }

    // Obtener el número de registro
    int registro_destino = atoi(register_token + 1); // +1 para omitir la letra "r"
    printf("-------------Registro destino: %d W\n", registro_destino);
...
void identify_instruction_and_operand(conf_object_t * obj, conf_object_t * cpu, logical_address_t address){ 
    ...
    bool available_station_multiply = false;
    ...
    if(strncmp(entry->disassembled_text, "mov", 3) == 0) {
        for(int i = 0; i < conn->total_reservation_stations; ++i){ //Add instruction to station
            if(!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_LOAD){
               ...
               conn->stations[i]->potenciaPorUso = LOAD_PW;
    ...
    if(strncmp(entry->disassembled_text, "add", 3) == 0) {//add, inc, dec
        ...
        int current_register = obtener_registro_destino(entry->disassembled_text);
        for (int i = 0; i < conn->issued_instruction_size; ++i) {
            if (conn->issued_instructions[i].order < entry->order &&
                conn->issued_instructions[i].dependent_register == current_register) {
                // Hay una dependencia de registro, la instrucción actual debe esperar
                conn->skip_instruction = true; // No ejecutar la instrucción actual todavía
                break;
            }
        }
        ...
        if (!conn->skip_instruction) {
            for(int i = 0; i < conn->total_reservation_stations; ++i){
                if(!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_SUM){
                    ...
                    conn->stations[i]->potenciaPorUso = SUM_PW;
    ...
    if (strncmp(entry->disassembled_text, "mul", 3) == 0) {
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_MULTIPLY) {
            	conn->stations[i]->curr_inst = entry;
            	conn->stations[i]->in_use = true;
            	conn->stations[i]->current_timeout = MULTIPLY_TIMEOUT;
                conn->stations[i]->potenciaPorUso = MULTIPLY_PW;
            	available_station_multiply = true;
            	conn->skip_instruction = true;
        	}
    	}
    }
    if (strncmp(entry->disassembled_text, "fmul", 4) == 0) {
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_MULTIPLY) {
            	conn->stations[i]->curr_inst = entry;
            	conn->stations[i]->in_use = true;
    ...
    if (strncmp(entry->disassembled_text, "div", 3) == 0) {
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_DIV) {
               ...
               conn->stations[i]->potenciaPorUso = DIV_PW;
    ...
    if (strncmp(entry->disassembled_text, "pop", 3) == 0) {
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_STR) {
                ...
                conn->stations[i]->potenciaPorUso = STR_PW;
    ...
    if (strncmp(entry->disassembled_text, "enter", 5) == 0) {
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_STR) {
            	conn->stations[i]->curr_inst = entry;
            	conn->stations[i]->in_use = true;
    ...
    if (strncmp(entry->disassembled_text, "xor", 3) == 0) {
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_XOR) {
                ...
                conn->stations[i]->potenciaPorUso = XOR_PW;
    ...
    if (strncmp(entry->disassembled_text, "call", 4) == 0) { //cmp, je, jmp, jg, jl, jge, jne, jz
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_CMP) {
                ...
                conn->stations[i]->potenciaPorUso = CMP_PW;
    ...
    if (strncmp(entry->disassembled_text, "jmp", 3) == 0) { //cmp, je, jmp, jg, jl, jge, jne, jz
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_CMP) {
            	...
            	conn->stations[i]->in_use = true;
            	conn->stations[i]->current_timeout = CMP_TIMEOUT;
                conn->stations[i]->potenciaPorUso = 5;
            	available_station_cmp = true;
            	conn->skip_instruction = true;
        	}
    	}
    ...
    if (strncmp(entry->disassembled_text, "loopne", 6) == 0) { //cmp, je, jmp, jg, jl, jge, jne, jz, loopne
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_CMP) {
            	conn->stations[i]->curr_inst = entry;
            	conn->stations[i]->in_use = true;
    ...
    if (strncmp(entry->disassembled_text, "jge", 3) == 0) { //cmp, je, jmp, jg, jl, jge, jne, jz, loopne
    	for (int i = 0; i < conn->total_reservation_stations; ++i) {
        	if (!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_CMP) {
            	conn->stations[i]->curr_inst = entry;
            	conn->stations[i]->in_use = true;
    ...
    if (strncmp(entry->disassembled_text, "<illegal instruction>", 21) == 0) { 
        available_station_syscall = true;
    	conn->skip_instruction = true;
    }

    //No station can take this instruction that was issued, we need to stall until they free up <illegal instruction>
    ...
    // Obtener el tiempo final y calcular la diferencia
    end_time = clock();
    ...
    printf("Tiempo transcurrido Total: %f segundos\n", cpu_time_total);
    ...
    for(int i = 0; i < conn->total_reservation_stations; ++i){
        if(conn->stations[i]->in_use && conn->stations[i]->current_timeout > 0) {
            ...
            if(conn->stations[i]->type==0){
                ...
                conn->stations[i]->potenciaPorUso += conn->stations[i]->potenciaPorUso;
                printf("Potencia consumida: %d W\n", conn->stations[i]->potenciaPorUso);
